...
- `defineClientTools` helper — define client tools once, derive types and server config from a single source
- `ClientEvent`, `ClientEventsFromTools`, `ClientToolDefinition` types exported from `@runwayml/avatars-react/api` for server-safe imports
