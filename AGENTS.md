...
- Test runner split: `package.json`'s `test` script uses vitest but test files import from `bun:test`, so `bun run test` / `npm test` errors — run tests with `bun test` directly. CI (`.github/workflows/ci.yml`) runs lint, typecheck, build, and `bun test` as four parallel jobs; Bun's test runner is very fast (~60 tests in ~500ms).
...
- Transcription transport: both `useTranscription` and `useTranscript` listen on `RoomEvent.DataReceived` alongside the native `RoomEvent.TranscriptionReceived` event, with parsing in shared `src/utils/parseTranscription.ts` that handles both JSON shapes the backend may send (LiveKit-style `{ segments: [...] }` and Runway flat `{ type: "transcription", role, turn, text }` streaming deltas from `worker:*` participants). Empirically the native LiveKit path never fires for Runway sessions today; `DataReceived` only carries `role: "assistant"` deltas (no user STT transcripts on the channel yet). Shipped in v0.13.0; closed issue #13.
