...
export type EventLogEntry =
  ...
  | { id: number; kind: 'rpc'; tool: string; args: Record<string, unknown>; durationMs: number; time: Date }
...
function entriesToText(entries: Array<EventLogEntry>): string {
  return entries
    .map((e) => {
      ...
      const prefix = e.kind === 'rpc' ? `[rpc ${e.durationMs}ms]` : '[client]';
      return `${formatTime(e.time)} ${prefix} ${e.tool} ${JSON.stringify(e.args)}`;
    })
    .join('\n');
}
  ...
  return (
    <div className="event-log">
      ...
      <div className="event-log-body" ref={scrollRef}>
        ...
        {entries.map((entry) => (
          <div key={entry.id} className={`event-log-row ${entry.kind === 'transcript' ? 'event-log-row-transcript' : ''} ${entry.kind === 'rpc' ? 'event-log-row-rpc' : ''}`}>
            ...
            {entry.kind === 'transcript' ? (
              ...
              <>
                ...
                <span className={`event-log-badge ${entry.kind === 'rpc' ? 'event-log-badge-rpc' : 'event-log-badge-client'}`}>
                  {entry.kind === 'rpc' ? 'rpc' : 'client'}
                </span>
                <span className="event-log-tool">{entry.tool}</span>
                {entry.kind === 'rpc' && <span className="event-log-duration">{entry.durationMs}ms</span>}
